...
class STSLoader:
    ...
    def load_file(self, filename: str) -> tuple[List[STSRecord], Dict[int, List[STSRecord]], int]:
        ...
        with open(filename, 'r') as file:
            ...
            for line in lines:
                ...
                self._create_sts_records(
                    sts_records, sts_table, sts_id, primer1, primer2, 
                    pcr_size, alias, line_no, bad_primers_ambig
                )
        ...
        logger.info(f"Loaded {len(sts_records)} STS records in {time.time() - start_time:.2f} seconds")
    ...
    def _create_sts_records(self, sts_records: List[STSRecord], sts_table: Dict[int, List[STSRecord]], 
                           sts_id: str, primer1: str, primer2: str, pcr_size: int, 
                           alias: str, line_no: int, bad_primers_ambig: int):
    ...
    def _insert_sts(self, sts_records: List[STSRecord], sts_table: Dict[int, List[STSRecord]], 
                    sts: STSRecord, hash_value: int):
