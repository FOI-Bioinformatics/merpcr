...
import tempfile
import os
from merpcr.core.utils import reverse_complement, hash_value, init_iupac_tables
...
def sts_record_data(draw):
    ...
    alias = draw(text(alphabet=string.ascii_letters + string.digits + "()- .", min_size=0, max_size=50))
...
class TestPropertyBasedUtils:
    ...
    def test_reverse_complement_base_validity(self, sequence):
        ...
        for base in result.upper():
            assert base in valid_bases or base == 'N', f"Invalid base {base} in result for {sequence}"
...
class TestPropertyBasedEngine:
    ...
    @given(integers(min_value=3, max_value=16),
    ...
    def test_merpcr_initialization_bounds(self, wordsize, margin, mismatches, 
                                         three_prime_match, iupac_mode, 
                                         default_pcr_size, threads):
    ...
    @given(text(alphabet=string.ascii_letters + string.digits + ">", min_size=1, max_size=50),
...
class TestPropertyBasedCLI:
    ...
    @given(lists(text(alphabet=string.ascii_letters + "=0123456789", min_size=1, max_size=10), 
    ...
    def test_parameter_types_bounds(self, value):
        ...
        from merpcr.cli import margin_type, mismatch_type, wordsize_type, threads_type
...
class TestPropertyBasedSequenceComparison:
    ...
    @given(dna_sequence(min_length=5, max_length=20),
